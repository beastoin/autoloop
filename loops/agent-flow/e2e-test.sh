#!/usr/bin/env bash
# E2E test for agent-flow against a live Flutter app.
# Usage: bash loops/agent-flow/e2e-test.sh <vm-service-uri>
...
WALKER="/tmp/agent-flow"
OUTPUT_DIR="/tmp/agent-flow-output"
...
cd "$WALKER" && node --experimental-strip-types src/cli.ts walk \
  --app-uri "$URI" --max-depth 1 --output-dir "$OUTPUT_DIR" 2>/tmp/agent-flow-stderr.log
WALK_EXIT=$?
