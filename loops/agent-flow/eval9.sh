#!/usr/bin/env bash
# Eval harness for agent-flow Phase 9 (migrate + pipeline).
# Usage: bash loops/agent-flow/eval9.sh
...
check "AC6: agent-flow migrate writes v2 YAML with --output" bash -c "
  tmpdir=\$(mktemp -d)
