# agent-flow Phase 2 — Run + Report (Open Source CLI v0.1)

## Goal

Extend agent-flow from a BFS explorer into an open-source CLI product that
**executes** YAML flow definitions and produces **interactive HTML reports**.

```
agent-flow run flow.yaml         # Execute flow → run.json + video + screenshots
agent-flow report run.json       # Generate self-contained HTML viewer
```

npm package: `agent-flow-cli` (MIT license)
...
```
agent-flow run <flow.yaml> [options]
...
agent-flow report <run.json> [options]
...
agent-flow-cli/
  ...
  tests/
    ...
    run-schema.test.ts
    ...existing tests...
  package.json          — name: agent-flow-cli, bin: agent-flow
  tsconfig.json
  LICENSE               — MIT
```
...
- [ ] `agent-flow run tab-navigation.yaml` executes all steps, produces run.json
