...
Applies Justin Poehnelt's "Rewrite your CLI for AI agents" principles to agent-flow.
...
- `hint` — actionable recovery suggestion ("Run: agent-flow schema run")
- `diagnosticId` — unique 8-char hex ID for log correlation

JSON error envelope: `{"error": {"code": "...", "message": "...", "hint": "...", "diagnosticId": "..."}}`

All throw sites in walker.ts, runner.ts, flow-parser.ts, agent-bridge.ts, capture.ts must use AgentFlowError, not raw Error/string.
...
Applied in cli.ts BEFORE command dispatch. Fail fast with AgentFlowError(INVALID_INPUT, ...).
...
```bash
agent-flow schema           # all commands (JSON array)
agent-flow schema walk      # single command schema
agent-flow schema run
agent-flow schema report
```
...
- Fail with structured AgentFlowError, not process.exit with console.error
...
1. `agent-flow schema` outputs valid JSON array with all 4 commands
2. `agent-flow schema run` outputs single command schema
3. `agent-flow run nonexistent.yaml` produces structured JSON error with code INVALID_INPUT and hint
4. `agent-flow run --dry-run flow.yaml` parses and resolves without executing
5. Non-TTY output is JSON by default (without --json flag)
6. `FLOW_WALKER_JSON=1 agent-flow walk --help` produces JSON, not human text
...
10. Every AgentFlowError has code, message, hint fields
11. No raw Error() or string throws remain in src/*.ts (all converted to AgentFlowError)
