# agent-flow — Automatic App Flow Extraction via agent-flutter

Build a CLI tool called `agent-flow` that connects to a running Flutter app via
...
1. **CLI entry point** — `agent-flow walk` command that accepts:
...
- [ ] `agent-flow walk --dry-run` connects, snapshots home screen, lists
      interactive elements and their safety status, exits without pressing
- [ ] `agent-flow walk --max-depth 1` explores one level deep: presses each
