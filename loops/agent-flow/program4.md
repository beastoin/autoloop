# Phase 4: Hosted reports — `agent-flow push`

## Objective

Add `agent-flow push <run-dir>` command that uploads a report to Cloudflare R2 and returns a shareable URL. One command, one file, one URL.

## Architecture

```
agent-flow push <run-dir>
  → generates report.html (if not already present)
  → uploads to R2: agent-flow-runs/<run-id>/report.html
...
### R1: CLI `push` command
- `agent-flow push <run-dir>` — uploads report and returns URL
...
### R4: CLI integration
- `push` appears in `agent-flow schema` output
- Input validation: run-dir must exist, must contain run.json
- Structured errors (AgentFlowError) for upload failures
...
1. `agent-flow push ./run-output/<id>/` uploads and returns a URL
2. URL serves the self-contained HTML report in a browser
3. `agent-flow push --json` returns `{"url", "id", "expiresAt"}`
4. `agent-flow schema` includes push command
