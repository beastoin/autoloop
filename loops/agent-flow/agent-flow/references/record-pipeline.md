...
Keep these rules:
- Pass `--json` on every `agent-flow` command.
...
```bash
agent-flow record stream --run-id "$RUN_ID" --run-dir "$RUN_DIR" --json < ./events.ndjson
```

Inline flag pattern:

```bash
agent-flow record stream \
  --run-id "$RUN_ID" \
  --run-dir "$RUN_DIR" \
...
Before finishing, capture raw logs as integrity evidence. agent-flow machine-synthesizes the timeline from these files — no manual annotation needed.
...
```bash
agent-flow verify ./flows/login.yaml --run-dir "$RUN_DIR" --recheck --json
agent-flow verify ./flows/login.yaml --run-dir "$RUN_DIR" --agent-prompt --json
agent-flow get "$RUN_ID" --json
```
...
```bash
agent-flow record stream --run-id "$RUN_ID" --run-dir "$RUN_DIR" --json < ./step-S1.ndjson
```
