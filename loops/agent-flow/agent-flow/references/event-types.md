...
**The primary way to get a correlated log timeline is to provide raw log files as evidence.** agent-flow machine-synthesizes the timeline — agents do NOT need to write synthesis manually.
...
agent-flow record finish --run-id $ID --run-dir $DIR --status pass

# report machine-synthesizes the timeline
agent-flow report $RUN_DIR

# JSON output includes the full timeline
agent-flow get $ID --json  # → { logTimeline: [...], ... }
```
