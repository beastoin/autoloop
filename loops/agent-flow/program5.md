...
Replace the `GET /` JSON status response with an HTML landing page showing live product metrics. Stats update on every `agent-flow push`. No frontend framework, no external deps.
...
3. Metrics update after each `agent-flow push`
