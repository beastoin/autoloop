#!/usr/bin/env bash
# Eval harness for agent-flow Phase 5 (Landing page with live metrics).
# Usage: bash loops/agent-flow/eval5.sh
...
WALKER_DIR="loops/agent-flow/agent-flow"
WORKER_DIR="loops/agent-flow/worker"
PASS=0
FAIL=0
...
echo "=== agent-flow Phase 5 eval ==="

# ── Gate 1: Phase 4 still passes ──
echo ""
echo "── Gate 1: Phase 4 regression ──"
check "Phase 4 eval passes" bash loops/agent-flow/eval4.sh
...
check "landing page has hero section" bash -c "
  grep -rqE 'hero|headline|agent-flow' '$WORKER_DIR/src/'
"
