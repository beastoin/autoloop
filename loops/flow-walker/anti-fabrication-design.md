# Anti-Fabrication Design for flow-walker

## Summary

`flow-walker` currently treats `events.jsonl` as ground truth. That is the core bug.
...
If `flow-walker` never witnesses the underlying transport, offline verification can only catch low-effort fakes. It cannot prove a run was real.
...
- Prevent fabrication when the host, `flow-walker`, and `agent-flutter` binaries are all fully compromised.
...
- `flow-walker` itself as a witnessing process
...
- `witnessed`: emitted by `flow-walker` or transport wrapper
- `derived`: computed by `flow-walker` from witnessed evidence
...
```json
{
  "schema": "flow-walker.event.v4",
  "event_id": "evt_01J...",
  "session_id": "ses_01J...",
  "origin": "witnessed",
  "source": "flow-walker|agent-flutter",
  "seq": 12,
...
If signatures are not available yet, phase 1 can use HMAC with a per-session secret owned by `flow-walker`, then upgrade to transport signatures in phase 2.

### 3. Introduce transport receipts

`flow-walker` should no longer trust an agent to describe a transport action. It must witness the action itself.
...
### 4. Make `flow-walker` or `agent-flutter` the witness

Long-term, the authoritative path should be:

1. `flow-walker` obtains a transport session from `agent-flutter`.
2. `flow-walker` issues or observes every transport command.
3. `flow-walker` writes authoritative receipt events.
4. the agent may still emit `note` and `agent-review`, but those are secondary.

This can be exposed in two ways:

- preferred: `flow-walker run` becomes the orchestrator and calls `AgentBridge`
- fallback: add `flow-walker witness agent-flutter ...` wrappers that agents must use instead of raw `agent-flutter`
...
```json
{
  "schema": "flow-walker.integrity.v1",
  "run_id": "25h7afGwBK",
  "flow_hash": "sha256:...",
  "session_id": "ses_01J...",
  "event_count": 84,
...
`flow-walker` already owns the run directory layout. It can hash every screenshot and video after capture.
...
- `flow-walker.event.v4`
- `flow-walker.run.v4`
- `flow-walker.integrity.v1`
...
### Phase 0: Immediate hardening in `flow-walker` only
...
### Phase 1: Witnessed recording path in flow-walker
...
- `record` path where `flow-walker` emits authoritative events for:
  - snapshot
  - command
  - screenshot
  - text query
...
- receipt schema shared between transport and `flow-walker`
...
That combination gives `flow-walker` a credible long-term anti-fabrication model while preserving offline verification.
